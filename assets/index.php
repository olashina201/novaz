...
if ($goin == 1) {
	...
	$dprw = mysqli_fetch_array($dpt);
	$lastid = $dprw['id'] + 1;

	$uname = $fn . ' ' . $ln;

	$transid = pin_generate();
	...
	$query = mysqli_query($link, "INSERT INTO 
		...
		VALUES('$transid', '$uid', '$lastid')
		");
	//echo $transid; echo '<br>'; echo $_SESSION['transid']; exit();			
...
function pin_generate()
{
	...
	$query = mysqli_query($link, "SELECT * FROM tempcode WHERE transid = '$generate_pin'");
